...
export default async function ApiKeysPage() {
  ...
  async function generateKey(formData: FormData) {
    ...
    await prisma.apiKey.create({ data: { storeId, userId: user.id, key, name } });
    ...
    <div>
      ...
      {stores.length === 0 ? (
        <div className="text-center py-20 bg-gray-50 rounded-2xl border border-gray-200">
          ...
          <h3 className="text-xl font-bold text-gray-500 mb-2">No stores connected</h3>
          <p className="text-gray-400">Add a store first to generate API keys.</p>
        </div>
      ) : (
          ...
          {stores.map((store) => (
            <div key={store.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
              <div className="p-6 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
                ...
                <form action={generateKey} className="flex gap-2">
              ...
              <div className="divide-y divide-gray-100">
                {store.apiKeys.length === 0 ? (
                  <p className="p-6 text-sm text-gray-400">No API keys generated yet.</p>
                ) : (
                  store.apiKeys.map((apiKey) => (
                    <div key={apiKey.id} className="p-4 flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-dark-navy">{apiKey.name}</p>
                        <code className="text-xs text-gray-400 bg-gray-50 px-2 py-0.5 rounded mt-1 inline-block">{apiKey.key}</code>
                      </div>
                      <div className="flex gap-2">
                        ...
                        <form action={revokeKey}>
                          ...
                          <button type="submit" className="p-2 hover:bg-red-50 rounded-lg text-red-400" title="Revoke"><Trash2 className="w-4 h-4" /></button>
                        </form>
                      </div>
                    </div>
                  ))
              ...
              {store.embedCode && (
                <div className="p-4 bg-dark-navy text-white text-xs font-mono overflow-x-auto">
                  {store.embedCode}
                  <button onClick={() => navigator.clipboard.writeText(store.embedCode || "")} className="ml-2 text-lemon-green hover:underline">Copy</button>
                </div>
              )}
            </div>
          ))}
