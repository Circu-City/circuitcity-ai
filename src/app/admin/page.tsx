  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="text-center">
        <div className="animate-spin w-10 h-10 border-2 border-lemon-green border-t-transparent rounded-full mx-auto mb-4" />
  ...
  if (!admin) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl p-8 shadow-lg max-w-md w-full text-center">
        ...
        <p className="text-sm text-gray-500 mb-6">Please log in with an admin account to access this page.</p>
        <button onClick={() => router.push("/login")} className="w-full py-3 rounded-xl bg-dark-navy text-white font-bold hover:opacity-90 transition-opacity">Go to Login</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-dark-navy text-white px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={handleLogout} className="flex items-center gap-1 text-sm text-red-400 hover:text-red-300"><LogOut className="w-4 h-4" /> Logout</button>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-8">
        <div>
          ...
          <p className="text-gray-500">Manage your application and clients</p>
        </div>
          ...
          {cards.map((card) => (
            <Link key={card.label} href={card.href} className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all block">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-2xl font-bold text-dark-navy">{card.value}</span>
              </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Link href="/admin/users" className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all group">
              ...
              <h3 className="font-semibold text-dark-navy mb-1">Manage Users</h3>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-dark-navy">Recent Users</h2>
            <Link href="/admin/users" className="text-sm text-lemon-green font-semibold hover:underline">View All →</Link>
          </div>
            ...
            <tbody>
              {(stats?.recentUsers || []).map((u: any) => (
                <tr key={u.id} className="border-b border-gray-50">
                  <td className="px-3 py-2.5 font-medium text-dark-navy">{u.name || "—"}</td>
                  <td className="px-3 py-2.5 text-gray-500">{u.email}</td>
                  <td className="px-3 py-2.5">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${u.role === 'admin' ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600'}`}>{u.role}</span>
                  </td>
                  <td className="px-3 py-2.5 text-gray-400">{new Date(u.createdAt).toLocaleDateString()}</td>
                </tr>
              ))}
              {(!stats?.recentUsers || stats.recentUsers.length === 0) && (
                <tr><td colSpan={4} className="px-3 py-8 text-center text-gray-400">No users yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
