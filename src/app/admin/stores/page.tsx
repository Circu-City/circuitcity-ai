    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto p-6">
        <div className="flex items-center justify-between mb-6">
          <div><h1 className="text-2xl font-bold text-dark-navy">All Stores</h1><p className="text-gray-500">{stores.length} stores total</p></div>
          <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search stores..." className="pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm w-64" /></div>
        </div>
            ...
            <tbody>
              {filtered.map(store => (
                <tr key={store.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-gray-100 text-gray-600">{store.plan}</span></td>
                  <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-700">{store.status}</span></td>
              ...
              {filtered.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">{search ? "No matches" : "No stores yet"}</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
