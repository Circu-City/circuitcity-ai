...
import { Store, ArrowLeft, Search, ExternalLink, MessageSquare, MoreHorizontal, Trash2, XCircle } from "lucide-react";
        ...
        {loading ? <div className="flex justify-center py-20"><div className="animate-spin w-10 h-10 border-2 border-lemon-green border-t-transparent rounded-full" /></div> : stores.length === 0 ? (
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <table className="w-full text-sm">
              <thead><tr className="bg-gray-50 border-b border-gray-100"><th className="text-left px-4 py-3 text-gray-500 font-medium">Store</th><th className="text-left px-4 py-3 text-gray-500 font-medium">Owner</th><th className="text-left px-4 py-3 text-gray-500 font-medium">Plan</th><th className="text-left px-4 py-3 text-gray-500 font-medium">Conv.</th><th className="text-left px-4 py-3 text-gray-500 font-medium">Status</th><th className="text-left px-4 py-3 text-gray-500 font-medium">Actions</th></tr></thead>
              <tbody>
                {filtered.map((store: any) => (
                  <tr key={store.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                    <td className="px-4 py-3">
                      <Link href={`/admin/stores/${store.id}`} className="font-medium text-dark-navy hover:text-lemon-green">{store.name || "(no name)"}</Link>
                      <div className="text-xs text-gray-400">{store.url || "(no url)"}</div>
                    </td>
                    <td className="px-4 py-3 text-gray-600">{store.ownerEmail || store.owner || "—"}</td>
                    <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${planColors[store.plan] || "bg-gray-100 text-gray-600"}`}>{store.plan || "starter"}</span></td>
                    <td className="px-4 py-3 text-gray-600 text-xs">{store.conversations || 0}</td>
                    <td className="px-4 py-3">
                      <button onClick={() => toggleStatus(store)} className={`px-2 py-0.5 rounded-full text-xs font-semibold ${statusColors[store.status] || "bg-gray-100 text-gray-600"} cursor-pointer hover:opacity-80`}>
                        {store.status || "pending"}
                      </button>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <Link href={`/admin/stores/${store.id}`} className="p-1.5 hover:bg-gray-100 rounded-lg text-gray-400" title="View Details"><MoreHorizontal className="w-4 h-4" /></Link>
                        {store.url && <a href={store.url} target="_blank" rel="noopener noreferrer" className="p-1.5 hover:bg-gray-100 rounded-lg text-gray-400"><ExternalLink className="w-4 h-4" /></a>}
                        <button onClick={() => deleteStore(store.id, store.name)} className="p-1.5 hover:bg-red-50 rounded-lg text-red-400"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
